(() => {
  ...
  function svgData(product) {
    ...
    let drawing = '';
    if (cat === 'perfumes') {
      drawing = `<rect x="116" y="88" width="68" height="26" rx="8" fill="${palette[2]}" opacity=".88"/><rect x="104" y="112" width="92" height="112" rx="24" fill="none" stroke="${palette[2]}" stroke-width="8"/><rect x="125" y="135" width="50" height="54" rx="12" fill="${palette[2]}" opacity=".24"/><circle cx="150" cy="162" r="12" fill="${palette[2]}" opacity=".75"/>`;
    } else if (cat === 'cremes') {
      drawing = `<ellipse cx="150" cy="126" rx="66" ry="24" fill="${palette[2]}" opacity=".9"/><path d="M84 126v78c0 24 132 24 132 0v-78" fill="${palette[2]}" opacity=".2" stroke="${palette[2]}" stroke-width="8"/><ellipse cx="150" cy="203" rx="62" ry="19" fill="${palette[2]}" opacity=".35"/><circle cx="150" cy="166" r="18" fill="${palette[2]}" opacity=".75"/>`;
    } else if (cat === 'acessorios') {
      drawing = `<rect x="103" y="72" width="94" height="170" rx="26" fill="none" stroke="${palette[2]}" stroke-width="8"/><rect x="117" y="92" width="66" height="104" rx="14" fill="${palette[2]}" opacity=".16"/><path d="M62 128c22-28 44-28 64 0M238 128c-22-28-44-28-64 0" fill="none" stroke="${palette[2]}" stroke-width="10" stroke-linecap="round"/><circle cx="62" cy="140" r="15" fill="${palette[2]}"/><circle cx="238" cy="140" r="15" fill="${palette[2]}"/>`;
    } else {
      drawing = `<rect x="95" y="50" width="110" height="210" rx="28" fill="#09090c" stroke="${palette[2]}" stroke-width="7"/><rect x="107" y="76" width="86" height="144" rx="13" fill="url(#screen)"/><rect x="132" y="60" width="36" height="6" rx="3" fill="${palette[2]}" opacity=".6"/><circle cx="150" cy="240" r="8" fill="${palette[2]}" opacity=".8"/>`;
    }

    const initials = escapeXml(product.name.split(/\s+/).slice(0, 2).map((x) => x[0]).join('').toUpperCase());
    const svg = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 300 270"><defs><linearGradient id="bg" x1="0" y1="0" x2="1" y2="1"><stop stop-color="${palette[0]}"/><stop offset="1" stop-color="${palette[1]}"/></linearGradient><linearGradient id="screen" x1="0" y1="0" x2="1" y2="1"><stop stop-color="${palette[2]}" stop-opacity=".8"/><stop offset="1" stop-color="#6fe1ff" stop-opacity=".35"/></linearGradient></defs><rect width="300" height="270" rx="32" fill="url(#bg)"/><circle cx="245" cy="42" r="80" fill="#fff" opacity=".06"/><circle cx="40" cy="235" r="90" fill="#fff" opacity=".035"/>${drawing}<text x="24" y="246" font-family="Arial,sans-serif" font-size="14" font-weight="700" fill="#fff" opacity=".72">${initials}</text></svg>`;
  ...
  function productImage(product) {
    return product?.image || svgData(product);
  }

  function wireImageFallbacks(root) {
    root.querySelectorAll('[data-product-image]').forEach((img) => {
      img.addEventListener('error', () => {
        const product = findProduct(img.dataset.productImage);
        if (product) img.src = svgData(product);
      }, { once: true });
    });
  }
  ...
  function renderCategories() {
    ...
    grid.innerHTML = categories.map((cat) => {
      ...
      return `<button class="category-card${active}" data-category="${cat.id}" type="button"><span class="cat-icon">${cat.icon}</span><strong>${escapeHtml(cat.name)}</strong><small>${count} produto${count === 1 ? '' : 's'} · ${escapeHtml(cat.subtitle)}</small></button>`;
    }).join('');
  ...
  function populateCategoryFilter() {
    $('categoryFilter').innerHTML = '<option value="all">Todas as categorias</option>' + categories.map((cat) => `<option value="${cat.id}">${escapeHtml(cat.name)}</option>`).join('');
  }
    ...
    grid.innerHTML = list.map((p) => {
      ...
      return `<article class="product-card">
        <div class="product-image">
          <img src="${escapeHtml(productImage(p))}" alt="Foto de exemplo de ${escapeHtml(p.name)}" loading="lazy" data-product-image="${escapeHtml(p.id)}" />
  ...
  function renderCart() {
    ...
    itemsEl.innerHTML = valid.map((item) => {
      ...
      return `<div class="cart-item">
        <div class="cart-thumb"><img src="${escapeHtml(productImage(p))}" alt="" data-product-image="${escapeHtml(p.id)}" /></div>
        <div class="cart-info">
          <strong>${escapeHtml(p.name)}</strong>
          <small>${money(p.price)} · subtotal ${money(Number(p.price) * item.qty)}</small>
          <div class="cart-qty"><button type="button" data-dec="${escapeHtml(p.id)}">−</button><span>${item.qty}</span><button type="button" data-inc="${escapeHtml(p.id)}">+</button></div>
        </div>
        <button class="remove-btn" type="button" data-remove="${escapeHtml(p.id)}" aria-label="Remover">×</button>
      </div>`;
    }).join('');

    wireImageFallbacks(itemsEl);
  ...
  function checkoutWhatsApp() {
    ...
    const message = ['Olá! Quero fazer um pedido na Lenda Motivação Store:', '', ...lines, '', `TOTAL: ${money(cartTotal())}`, '', 'Por favor, confirme disponibilidade, entrega e forma de pagamento.'].join('\n');
